...
public class Main extends Application {
    ...
    // Buttons
    private Button btAddAgent = new Button("Thêm tác tử");
    private Button btAddDirtAgent = new Button("Add Dirt Agent");
    private Button btAddWall = new Button("Vẽ tường");
    private Button btAddDirt = new Button("Add Dirt");
    private Button btStart = new Button("Bắt đầu");
    private Button btStop = new Button("Dừng lại");
    ...
    private Button btClear = new Button("Xóa bản đồ");
    private Button btRandomDirt = new Button("Random ô bẩn");
    ...
    public void start(Stage primaryStage) {
        ...
        rbFullyObservable = new RadioButton("Quan sát toàn phần");
        rbPartiallyObservable = new RadioButton("Quan sát một phần");
        ...
        observability.selectedToggleProperty().addListener((ov, old_toggle, new_toggle) -> {
            ...
            if (observability.getSelectedToggle() == rbFullyObservable) {
                ...
                cbAlgorithm.getItems().addAll("Dijkstra", "Minimax", "Alpha Beta");
        ...
        rbStatic = new RadioButton("Tĩnh");
        rbDynamic = new RadioButton("Động");
        ...
        flowPane.getChildren().addAll(new Label("Observability"), rbFullyObservable, rbPartiallyObservable,
                new Label("Môi trường"), rbStatic, rbDynamic);

        cbAlgorithm.setId("comboBox");
        cbAlgorithm.getItems().addAll("Dijkstra", "Minimax", "Alpha Beta");
        cbAlgorithm.setPromptText("Lựa chọn thuật toán");
        ...
        primaryStage.setTitle("Công nghệ tác tử _ Máy hút bụi");
        primaryStage.show();
    }
    ...
    private void runClock() {
        running=true;new Thread(()->{long last=System.nanoTime();double delta=0;double ns=1_000_000_000.0/1;count=0;String str=String.format("Time Elapsed: %d s",count);clock.refreshDigits(str);

        while(running){long now=System.nanoTime();delta+=(now-last)/ns;last=now;

        while(delta>=1){count++;str=String.format("Time Elapsed: %d s",count);clock.refreshDigits(str);if(count==timer)stopAnimation();

        delta--;}}}).start();
    }

    private void playAnimation() {
        if(cbAlgorithm.getSelectionModel().getSelectedItem()==null){JOptionPane.showMessageDialog(null,"Vui lòng lựa chọn thuật toán","Warning!",JOptionPane.WARNING_MESSAGE);return;}

        if(agentList.isEmpty()){JOptionPane.showMessageDialog(null,"Please add agent.","Warning!",JOptionPane.WARNING_MESSAGE);return;}

        for(Agent agent:agentList)agent.initVisited(nbRows,nbColumns);for(DirtAgent agent:dirtAgentList)agent.initVisited(nbRows,nbColumns);disableButtons();

        runClock();System.out.println("New Animation Timer created");animationTimer=new AnimationTimer(){int fps=6; // number
                                                                                                                    // of
                                                                                                                    // update
                                                                                                                    // per
                                                                                                                    // second.
        double tickPerSecond=1_000_000_000.0/fps;double delta=0;long lastTime=System.nanoTime();

        @Override public void handle(long now){now=System.nanoTime();delta+=(now-lastTime)/tickPerSecond;lastTime=now;
        // System.out.printf("Delta %.2f%n", delta);

        if(delta>=1){

        if(!dynamic&&PathFinder.getDirtTilesFullyObs(map,nbRows,nbColumns).isEmpty()){stopAnimation();}

        // DONT CHANGE ORDER!!!
        for(Agent agent:agentList)agent.update(map,nbRows,nbColumns);

        for(DirtAgent agent:dirtAgentList)agent.update(map,nbRows,nbColumns);
        // DONT CHANGE ORDER!!!
    ...
    public void stopAnimation() {
        try {
            ...
            animationTimer.stop();
            enableButtons();
            JOptionPane.showMessageDialog(null, "Số ô: " + nbofTiles
                    + "\nSố ô bẩn: "
                    + (nbOfCleanedTiles + PathFinder.getDirtTilesFullyObs(map, nbRows, nbColumns).size())
                    + "\nSố ô được làm sạch : " + nbOfCleanedTiles
                    + "\nSố ô còn lại " + PathFinder.getDirtTilesFullyObs(map, nbRows, nbColumns).size()
                    + "\nSố bước di chuyển : " + cleanPathSize,
                    "Hiệu suất", JOptionPane.INFORMATION_MESSAGE);
            nbOfCleanedTiles = 0;
            cleanPathSize = 0;
            dirtPathSize = 0;
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Quá trình chưa bắt đầu!",
                    "Hiệu suất", JOptionPane.WARNING_MESSAGE);
        }
